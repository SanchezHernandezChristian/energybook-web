    ...
    <div id="side-nav" class="menu d-none d-sm-block d-xs-block">
      ...
      <b-nav vertical class="w-100">
        ...
        <b-nav-item
          ...
          v-bind:class="{
            'current-view': currentView === 'companies' || currentView === 'companyDetail' || currentView === 'companyProfile',
          }"
...
export default {
  ...
  computed: {
    ...
    companyId() {
      // eslint-disable-next-line vue/no-side-effects-in-computed-properties
      return (this.$store.state.companyId = JSON.parse(localStorage.getItem('user')).company_id);
    },
    isAdmin() {
      ...
      designatedMeters
        ...
        .then((designatedMeters) => {
          ...
          if (designatedMeters[0].tipo == 'Acuvim II') {
            ...
            this.services = designatedMeters[0].devices.map((service) => service.name);
