<template>
  <b-row id="customzie" class="main">
    ...
    <b-col lg="7">
      <b-card class="information-files" no-body>
        <b-row class="card-header">
          <b-col>
        ...
        <b-row class="card-body">
          ...
          <b-col>
            ...
            <v-table :items="formattedInfoFiles" :fields="infoFields" @delete="deleteFile"> </v-table>
          </b-col>
        </b-row>
      </b-card>
    </b-col>
...
export default {
  ...
  beforeMount() {
    informationFile.find({}).then((files) => {
      files.forEach((file) => {
        this.addNewPdf(file);
      });
    });
