<template>
  <div>
    <button @click="$bvModal.show(DataNotificacion.id)" title="Click para ver notificacion" variant="primary" class="btn">
    ...
    <b-modal :id="DataNotificacion.id">
      ...
      <div>
        <b-container class="Dispositivos">
          <b-row>
            <b-col cols="6">
              <p class="text-center" v-for="Dispositivos in this.arregloNombres" :key="(Dispositivos, index)">
                <strong>{{ Dispositivos }} </strong>
              </p>
            </b-col>
            <b-col cols="3">
              <p class="text-center" v-for="valores in this.arreglovalores" :key="(valores, index)">{{ valores }}</p>
            </b-col>
          </b-row>
        </b-container>
      </div>

      <p class="text-center"><strong> Servicios </strong></p>
      <br />
      ...
      <div>
        <b-container class="Servicios">
          <b-row>
            <b-col cols="6">
              <p class="text-center" v-for="Servicios in this.arregloservicios" :key="(Servicios, index)">
                <strong> {{ Servicios }} </strong>
              </p>
            </b-col>
            <b-col cols="3">
              <p class="text-center" v-for="valoresServicios in this.arregloValoresServicios" :key="(valoresServicios, index)">{{ valoresServicios }}</p>
            </b-col>
          </b-row>
        </b-container>
      </div>
    ...
    this.DataNotificacion.Dispositivos.forEach((element) => {
      ...
      if (res.length == 3) {
        if (res[2].charAt(0) == '$') {
          ...
          // si no es de costo pero tiene 3 valores
