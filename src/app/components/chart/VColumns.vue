  ...
  methods: {
    ...
    getData(filter, interval, chart) {
      ...
      if (this.$store.state.mode == 'ACUVIM') {
        Minutesss.ConsumptionCostFilter(meter[1], filter, interval).then((res) => {
          if (res) {
            console.log(res);
            let data = [];
            let tickInterval;
        ...
        meters
          .getConsumptionCostsByFilter(meter_id, meter_device, service, filter, interval, this.date_custom)
          .then((res) => {
            if (res) {
              console.log(res);
              let data = [];
